...
export function OnboardingForm({ profile, randomMovie, email }: { profile: UserProfile | null; randomMovie: { backdrop_url: string; title: string } | null; email?: string; }) {
    ...
    // ✨ 1. Prepopulation with "Smart Overwrite" Logic
    useEffect(() => {
        let derivedUsername = '';
        ...
        if (email) {
            ...
            // Remove special chars to make a clean username (e.g., "john.doe" -> "johndoe")
            derivedUsername = emailPrefix.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase();
            // Format display name (e.g. "john.doe" -> "John Doe")
        ...
        // HEURISTIC: Check if the existing username is the default "gibberish" (usually user_ + 8 chars)
        // If it starts with "user_" or is empty, we consider it unset and use the derived one.
        ...
        // ✨ Trigger check immediately if we populated a valid username
